...
public class TeachersController {
    ...
    public String tasks(Model model, @ModelAttribute("subTasks") ArrayList<SubTaskReadDto> subTasks, TaskCreateEditDto taskCreateEditDto, HttpSession httpSession, Pageable pageable){
        ...
        if (!subTasks.isEmpty()){
            ...
            TaskCreateEditDto taskCreateEditDto1=new TaskCreateEditDto(taskReadDto.getType(), taskReadDto.getTask_header(), taskReadDto.getEndTime(), taskReadDto.getUser().getId());
            taskReadDto.getSubTaskReadDtos().stream().forEach(subTaskReadDto -> taskCreateEditDto1.add(new SubTaskCreateEditDto(subTaskReadDto.getSubtask(), subTaskReadDto.getTaskId(), subTaskReadDto.getStatus())));
